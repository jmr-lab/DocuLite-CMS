...
class JpCopyPrecondition
{
	 ...
	 */
	public function JpCopyPrecondition() {}
	...
	public function queryExecute($session, $object)
	{
		...
		// Case object is locked by someone else
		$userName = array($user->getValue('user_name'), $user->getValue('user_login_name'), $user->getValue('r_object_id'));
		if ($object->getValue('r_lock_owner') <> '' && !in_array($object->getValue('r_lock_owner'), $userName))	{return false;}
		// Otherwise
		return ($object->getValue('r_accessor_permit') <= 3 && $user->getValue('user_name') <> $object->getValue('owner_name')) ? false : true;
	}
}
?>