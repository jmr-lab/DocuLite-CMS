 ...
 * @version		3.0
 */
class JwBookMark extends JwModalList
{
	/**
	 * List of columns
	 *
	...
	public function init()
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT s1.r_object_id, s1.r_object_type, s1.object_name,
				...
				WHERE	s1.i_is_deleted = false
						...
						AND (r_accessor_permit >= 3 OR s1.owner_name = '".$user->getValue('user_name')."')
	...
	public function onOk()
	{
		...
		foreach ($objectIds as $key => $strObjectId)
		{
			$auditTrailMgr->createAudit($strObjectId, 'bookmark', $stringArgs, null);
		}
		return '';
	}
}
