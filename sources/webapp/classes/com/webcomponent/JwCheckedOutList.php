 ...
 * @version		3.0
 */
class JwCheckedOutList extends JwObjectList
{
	/**
	 * Returns the title image (Checked Out).
	 *
	 * @access	public
	...
	/**
	 * Returns the title name (Checked Out).
	 *
	 * @access	public
	 * @return	String	the title name
	 */
	...
	{
		JcLogger::debug(__CLASS__.'.'.__FUNCTION__.'()');
		return $this->getString('CHECKED_OUT');
	}
	...
	public function init()
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT jm_sysobject_s.r_object_id, r_object_type, object_name, owner_name, r_creation_date, r_modify_date,
				...
				WHERE	jm_sysobject_s.i_is_deleted = false
						...
						AND (r_lock_owner = '".$user->getValue('r_object_id')."'
						OR r_lock_owner = '".$user->getValue('user_name')."')";
