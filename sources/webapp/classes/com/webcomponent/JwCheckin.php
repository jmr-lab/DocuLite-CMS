 ...
 * @version		3.0
 */
class JwCheckin extends JwModalList
{
	/**
	 * List of columns
	 *
	 ...
	 * @var		array
	 */
	protected $columns = array('checkout', 'icon', 'object_name', 'file', 'version');
	...
	protected function setLinks()
	{
		...
		{
//			$link['open'] = '<input type="hidden" name="objectId['.$object->getValue('r_object_id').']" value="'.$object->getValue('r_object_id').'"><span style="font-size: 12px; font-weight: bold; color: #5F5F5F;">';
			$link['open'] = '<span style="font-size: 12px; font-weight: bold; color: #5F5F5F;">';
			$link['close'] = '</span>';
			// Set the link
	...
	public function init()
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT s1.r_object_id, s1.r_object_type, s1.object_name,
						s1.owner_name, s1.r_creation_date, s1.r_modify_date, s1.r_immutable_flag, r1.r_version_label,
	...
	public function onOk()
	{
		...
		$arrObjectIds = $request->getParameter('objectId');
		$arrVersions = $request->getParameter('version');
		$arrFiles = $request->getParameter('file');
		$strObjectIds = implode($arrObjectIds, "', '");
		// Use of JfCheckinOperation
		$checkinOperation = new JfCheckinOperation();
		foreach ($arrObjectIds as $key => $strObjectId)
		{
//			JcLogger::info('CHK - strObjectId : '.$strObjectId);
			$strVersion = $arrVersions[$strObjectId];
//			$strFileName = _SERVER_ROOT_."/temp/".$session->getLoginUserId()."/".$arrFiles[$strObjectId];
			$strFileName = "/temp/".$session->getLoginUserId()."/".basename($arrFiles[$strObjectId]);
			// JcLogger::info('strFileName : '.$strFileName);
			// Use of JfCheckinOperation
			$checkinOperation->setCheckinVersion($strVersion);
			if (basename($arrFiles[$strObjectId]) <> '')	$checkinOperation->setCheckinFile($strFileName);
			$checkinObject = $session->getObject(new JfId($strObjectId));
			// $checkinOperation->setVersionLabels("DRAFT,WIP");
			$checkinOperation->add($checkinObject);
		}
