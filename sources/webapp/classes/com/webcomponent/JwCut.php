 ...
 * @version		3.0
 */
class JwCut extends JwModalList
{
	/**
	 * List of columns
	 *
	...
	public function init()
	{
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT s1.r_object_id, s1.r_object_type, s1.object_name,
				...
				WHERE	s1.i_is_deleted = false
						...
						AND (r_accessor_permit >= 6 OR s1.owner_name = '".$user->getValue('user_name')."')
...
//		JcLogger::info("strObjectIds : '".$strObjectIds."'");
		...
		foreach ($objectIds as $key => $strObjectId)
		{
			...
			$clipboardObject->setOperation('cut');
		...
		// Get the user login info
		$user = $this->user;
		// Create a cut event for each object
		...
		foreach ($objectIds as $key => $strObjectId)
		{
			$auditTrailMgr->createAudit($strObjectId, 'cut', $stringArgs, null);
		}
		return '';
	}
}
