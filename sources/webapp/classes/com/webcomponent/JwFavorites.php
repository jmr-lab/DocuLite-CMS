 ...
 * @version		3.0
 */
class JwFavorites extends JwObjectList
{
	/**
	 * List of columns
	 *
...
//	protected $columns = array('icon', 'object_name', 'properties', 'r_creator_name', 'r_start_date');
	...
	{
		JcLogger::debug(__CLASS__.'.'.__FUNCTION__.'()');
		return $this->getString('FAVORITES');
	}
	...
	public function init()
	{
		...
		$bookmarkIds = $favorites->getObjectIds();
		$strBookMarkIds = implode("', '", $bookmarkIds);
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT jm_sysobject_s.r_object_id, r_object_type, object_name, owner_name, r_creation_date, r_modify_date,
				...
				WHERE	jm_sysobject_s.i_is_deleted = false
						...
						AND jm_sysobject_s.r_object_id IN ('".$strBookMarkIds."')";
