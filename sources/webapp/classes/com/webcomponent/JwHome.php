 ...
 * @version		3.0
 */
class JwHome extends JwDocList
{
	/**
	 * Returns the title name (Home).
	 *
	 * @access	public
	...
	public function getTitlePath()
	{
		...
		// Init the path
		$path = new JcPath();
		$this->path = $path->getFullPath($this->folderId);
//		foreach ($this->path as $key => $value)	{JcLogger::info('Path['.$key.'] : '.$value);}
		$listIDs = $this->path;
		$this->path = array();
		parent::initPath($listIDs);
//		foreach ($this->path as $key => $value)	{JcLogger::info('Path['.$key.'] : '.$value);}
		return parent::getTitlePath();
	}
	...
	public function init()
	{
		...
		$this->folderId = (($user->getValue('default_folder') == '') ? $session->getDocbaseId() : $user->getValue('default_folder'));
		// Init the path
		$path = new JcPath();
		$pathIds = $path->getFullPath($this->folderId);
		array_pop($pathIds);
		$this->initPath($pathIds);
		...
		$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT jm_sysobject_s.r_object_id, r_object_type, object_name, owner_name, r_creation_date, r_modify_date,
				...
				WHERE	jm_sysobject_s.i_is_deleted = false
						...
						AND r2.i_folder_id = '".$this->folderId."'
